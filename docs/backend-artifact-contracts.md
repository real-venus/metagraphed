...
- `/metagraph/chain/census.json`: schema for the network registration/population census across every subnet's neurons — neuron/subnet counts, the active/immunity/validator/miner split and rates, and the registration-age distribution — computed live from the `neurons` D1 tier at `GET /api/v1/chain/census` (no static file).
...
- `/api/v1/chain/census`: fetch the network registration/population census across every subnet's neurons — neuron/subnet counts, the active/immunity/validator/miner split and rates, and the registration-age distribution (computed live from the `neurons` D1 tier; schema-stable nulls when cold).
